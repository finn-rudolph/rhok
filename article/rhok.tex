...
Für $\E(t_{\min}) \sim F$ wird Induktion über $M$ verwendet. Die Induktionsvoraussetzung ist jedoch stärker: Sie besagt, dass eine (von $M$ abhängige) Konstante $c$ existiert, sodass $|\E(t_{\min}) - F| \le c$ für jedes $p$. Sei
Im Fall $M = 1$ gilt
\begin{align*}
    \lim_{p \to \infty} \frac F {\E(t_{\min})}
     & = \lim_{p \to \infty} 1 + \frac {F - \E(t_{\min})} {\E(t_{\min})} \\
