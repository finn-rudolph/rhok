...
\section{Herleitung einer Formel für die Laufzeit}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \frac {z_1} {h_1} \exp \bigg ( \frac {-z_1^2}{2h_1} \bigg ) dz_1          \\
            & = p^M \exp \bigg (\frac {-h_1} 2 \bigg ) \to 0 \quad (p \to \infty)
    \end{align*}
    Daraus folgt
    \begin{align*}
        F \sim
        \int_0^{\infty}
        \frac {z_1} {h_1} \exp \bigg ( \frac {-z_1^2} {2h_1} \bigg ) \cdots
        \int_0^{\infty}
        \frac {z_M} {h_M} \exp \bigg ( \frac {-z_M^2} {2h_M} \bigg )
...
\begin{proof}[Beweis von Lemma \ref{lemma:prob-s-z-asmyp}]
    Zunächst wird die Restgliedabschätzung $\exp(x) = 1 + x + O(x^2)$ für $|x| \le 1$ auf $\P(s = z)$ angewandt. Damit gilt
    \begin{align*}
        \P(s = z) = \frac z h \prod_{j = 0}^{z - 1}
        \Bigg ( \exp \bigg ( \frac {-j} h \bigg )
...
Um $\E(t_{\min}) \sim F$ zu zeigen, wird Induktion über $M$ verwendet. Die Induktionsvoraussetzung ist, dass $|\E(t_{\min}) - F| = O(p^{\epsilon})$ für jedes $\epsilon > 0$. Daraus folgt $\E(t_{\min}) \sim F$, denn nach Lemma \ref{lemma:f-asmyp} gilt $F = \Theta(\sqrt p)$.

Für den Induktionsanfang, sei $M = 1$ und $\epsilon > 0$. Man wähle $\delta = \min(\epsilon/2, 1/6)$. Es gilt
\begin{align*}
    |\E(t_{\min}) - F|
     & = \Bigg \vert
    \sum_{z_1 = 1}^{h_1} \big (\P(s_1 = z_1) - Q(z_1, h_1) \big ) z_1
    \Bigg \vert                                \\
     & \le \sum_{z_1 = 1}^{h_1^{1/2 + \delta}}
    \big | \P(s_1 = z_1) - Q(z_1, h_1) \big | z_1 +
    \sum_{z_1 = h^{1/2 + \delta}}^{h_1}
    \big ( \big | \P(s_1 = z_1) \big | + \big | Q(z_1, h_1) \big | \big ) z_1
\end{align*}
Indem man Lemma \ref{lemma:prob-s-z-asmyp} auf die erste Summe anwendet, erhält man folgende Abschätzung.
\begin{align*}
    \sum_{z_1 = 1}^{h_1^{1/2 + \delta}}
    \big | \P(s_1 = z_1) - Q(z_1, h_1) \big | z_1
     & =
    O \Bigg (
    \sum_{z_1 = 1}^{h_1^{1/2 + \delta}} \frac {z_1} {h^{1-4\delta}} \Bigg ) \\
     & = O \Bigg ( h_1^{1/2 + \delta} \,
    \frac {h_1^{1/2 + \delta}} {h_1^{1 - 4\delta}} \Bigg )                  \\
     & = O \Big ( p^{2\delta}\Big )                                         \\
\end{align*}
Da $\P(s_1 = z_1) = O(Q(z_1, h_1))$, kann die zweite Summe wie folgt abgeschätzt werden.
\begin{align*}
    \sum_{z_1 = h^{1/2 + \delta}}^{h_1}
    \big ( \big | \P(s_1 = z_1) \big | +
    \big | Q(z_1, h_1) \big | \big ) z_1
     & =
    O \Bigg ( \sum_{z_1 = h_1^{1/2 + \delta}}^{h_1}
    \frac {z_1^2}{h_1} e^{-z_1^2 / (2h_1)} \Bigg )       \\
     & = O \Bigg ( \sum_{z_1 = h_1^{1/2 + \delta}}^{h_1}
    h_1 e^{-h_1^{2\delta}/2} \Bigg )                     \\
     & = O \Big ( h_1^2 e^{-h_1^{2 \delta}/2} \Big )
    \to 0 \quad (p \to \infty)
\end{align*}
Das schließt den Beweis des Induktionsanfangs ab. Sei nun $M \in \N, M \ge 2$.

\begin{align*}
    \lim_{p \to \infty} \frac {F} {\E(t_{\min})}
     & = \lim_{p \to \infty}
    1 + \frac{F - \E(t_{\min})} {\E(t_{\min})}                               \\
     & = 1 + \lim_{p \to \infty} \frac 1 {\E(t_{\min})}
    \sum_{z_1 = 1}^{h_1} \bigg ( Q(z_1, h_1) - \P(s_1 = z_1) \big ) \cdots   \\
     & \qquad \sum_{z_M = 1}^{h_M} \big ( Q(z_M, h_M) - \P(s_M - z_M) \big )
    \min_{i = 1}^M(z_i \lg 2k_i)
\end{align*}
Es wird gezeigt, dass der Limes in obiger Formel 0 ist. Dafür reicht es zu zeigen, dass die verschachtelte Summe im Limes (alles unter dem Limes außer $1/\E(t_{\min}))$ durch eine Konstante beschränkt ist, da $\lim_{p \to \infty} \E(t_{\min}) = \infty$. Dafür wird der Betrag betrachtet und $\min_{i = 1}^M (z_i \lg 2k_i)$ durch $z_1 \lg 2k_1$ abgeschätzt.
\begin{align*}
        & \Bigg \vert
    \sum_{z_1 = 1}^{h_1} \big ( Q(z_1, h_1) - \P(s_1 = z_1) \big ) \cdots
    \sum_{z_M = 1}^{h_M} \big ( Q(z_M, h_M) - \P(s_M - z_M) \big )
    \min_{i = 1}^M(z_i \lg 2k_i)
    \Bigg \vert                                                 \\
    \le & \sum_{z_1 = 1}^{h_1} | Q(z_1, h_1) - \P(s_1 = z_1) |
    z_1 \lg 2k_1
    \sum_{z_2 = 1}^{h_2} | Q(z_2, h_2) - \P(s_2 = z_2) | \cdots \\
        & \qquad
    \sum_{z_M = 1}^{h_M} | Q(z_M, h_M) - \P(s_M - z_M) |
\end{align*}
