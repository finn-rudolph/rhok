...
In diesem Abschnitt wird die Frage behandelt, wie der Paramter $k$ bei $M$ Maschinen bestmöglich gewählt wird. Mit Formel (\ref{lkp}) und Satz 2 konnten Ergebnisse in den Fällen $M = 1$ und $M = 2$ erzielt werden. Es wird noch folgendes Lemma benöigt.
...
\begin{proof}
    ...
    \end{align*}
    was für $k \ge 4$ die gewünschte Ungleichung liefert. Durch Rechnung erhält man $\E(L_2(p)) \approx \sqrt{\pi p / 2} \cdot 3.1547$ und $\E(L_3(p)) \approx \sqrt{\pi p /2} \cdot 5.4508$, woraus die Optimaität von $k = 1$ folgt.
\end{proof}
