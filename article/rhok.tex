...
\begin{align*}
    E(K) = \E \bigg ( \min_{1 \le i \le M} X_i \bigg )
\end{align*}
wobei $X_i$ die Zufallsvariable ist, die dem Anfangswert der $i$-ten Maschine die Zeit bis zum Auftreten eine Kollision zuordnet.

\section{Eine Formel für unabhängige Maschinen}

In diesem Abschnitt wird eine Formel für $E(K)$ hergleitet werden, die im Fall paarweise verschiedener $k$-Werte gilt. Der Analyse der Rho-Methode liegt die Annhame zugrunde, dass sich die Abbildung $f : x \mapsto x^{2k} + 1$ bezüglich der durchschnittlichen Rho-Länge wie eine zufällig gewählte Abbildung verhält (\cite{pol75}, S. 332). Für den Fall mehrerer Maschinen postulieren wir zusätzlich, dass $X_i$ von $X_j$ stochastisch unabhängig ist, wenn $k_i \ne k_j$. Für $k_i = k_j$ gilt das nicht, da die zwei Maschinen die selbe Funktion $f$ iterieren und nur die Anfangswerte unabhängig sind. Wenn aber $k_i \ne k_j$ und sich $x \mapsto x^{2k_i} + 1$ und $x \mapsto x^{2k_j} + 1$ bezüglich der Rho-Länge wie zufällig gewählte Abbildung verhalten, ist es vertretbar, die Unabhängigkeit von $X_i$ und $X_j$ anzunehmen. Das kann experimentell unerstützt werden (siehe Abschnitt \ref{sec:ex}).

Folgendes Argument basiert auf dem von \cite{bp81}, S. 627 -- 628, angepasst für mehrere Maschinen. Wir bezeichnen mit $\Z/p\Z^*$ die Gruppe der Einheiten in $\Z/p\Z$. Man beobachte, dass $\psi : \Z/p\Z^\times \to \Z/p\Z^\times$ mit $\psi : x \mapsto x^{2k}$ ein Gruppenendomorphismus mit $[\Z/p\Z : \text{Im} f] = \gcd(p - 1, 2k)$ ist. Daher haben genau $(p - 1) / \gcd(p - 1, 2k)$ Knoten in $\gamma(f)$ Eingangsgrad $\gcd(p - 1, 2k)$, und alle anderen Eingangsgrad 0. Knoten 1 als Einzelfall mit Eingangsgrad ignorieren wir. Wir wollen nun die die Wahrscheinlichkeit betrachten, dass für eine Algorithmus nach genau $t$ Zeiteinheiten terminiert, wobei eine Zeiteinheit die Dauer einer Iteration im Fall $k = 1$ ist. Eine Maschine mit Parameter $k$ schafft in $t$ Zeiteinheiten also $\lfloor t / \lg 2k \rfloor$ Iterationen. Die Wahrscheinlichkeit, dass im $i$-ten Schritt zu einem Vorgänger eines bere der erste Zyklus nach $t$ Schritten gefunden wird, ist
\begin{align*}
    \prod_{i = 1}^M \prod_{j = 0}^{\lfloor t / \lg 2k_i \rfloor -1} \bigg ( 1 - \frac {i} {p / (\gcd(p - 1, 2k) - 1)}\bigg)
\end{align*}
