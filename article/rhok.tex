...
\begin{align*}
    \P(s_i = s)
     & = \P(s_i > s - 1) - \P(s_i > s)                                                                                 \\
     & = \prod_{j = 0}^{s - 1} \bigg (1 - \frac {j} {h_i} \bigg ) - \prod_{j = 0}^s \bigg (1 - \frac {j} {h_i} \bigg ) \\
     & = \frac s {h_i} \prod_{j = 0}^{s - 1} \bigg (1 - \frac {j} {h_i} \bigg )
\end{align*}
Nun wird die Restgliedabschätzung $\exp x = 1 + x + O(x^2)$ für $|x| < 1$ mit $x = -j/h_i$ angewandt.
\begin{align}
    \P(s_i > s) = \prod_{j = 0}^s \Bigg ( \exp \bigg ( \frac {-j}{h_i} \bigg ) - O \bigg ( \frac {j^2} {h_i^2} \bigg ) \Bigg ) \qquad
    \P(s_i = s) = \frac s {h_i} \prod_{j = 0}^{s - 1} \Bigg ( \exp \bigg ( \frac {-j}{h_i} \bigg ) - O \bigg ( \frac {j^2} {h_i^2} \bigg ) \Bigg )
    \label{ps}
\end{align}
Damit kann $\E(t_{\min})$ wie folgt ausgedrückt werden.
\begin{align}
    \E(t_{\min}) = \sum_{r_1 = 1}^{h_1} \P(s_1 = r_1) \sum_{r_2 = 1}^{h_2} \P(s_2 = r_2) \, \cdots \sum_{r_M = 1}^{h_M} \P(s_M = r_M) \, \min_{i = 1}^M (r_i \lg 2k_i) \label{etmin}
\end{align}
Die Behauptung ist nun, dass zur Bestimmung einer asymptotischen Näherung von $\E(t_{\min})$ die $O(j^2/h_i^2)$-Terme in $\P(s_i = s)$ weggelassen werden können. Intuitiv ist der Grund dafür, dass wenn $r_i$ deutlich kleiner als $h_i$ ist, auch $j$ deutlich kleiner als $h_i$ und damit $j^2/h_i^2$ nahe 0 ist. Wenn dagegen $r_i$ nahe $h_i$ ist, ist $\P(s_i = r_i)$ sehr klein, sodass der Beitrag zum Erwartungswert vernachlässigbar ist. Präzise lässt sich das wie folgt begründen. Wenn $r_i \le h_{i}^{3/5}$, sieht man durch Ausmultiplizieren des Produkts in (\ref{ps})
\begin{align*}
         & \Bigg \vert \frac {r_i} {h_i} \prod_{j = 0}^{r_i - 1} \Bigg ( \exp \bigg ( \frac {-j}{h_i} \bigg ) - O \bigg ( \frac {j^2} {h_i^2} \bigg ) \Bigg ) - \frac {r_i} {h_i} \prod_{j = 0}^{r_i - 1} \exp \bigg ( \frac {-j}{h_i} \bigg ) \Bigg \vert \\
    = \, & \frac {r_i} {h_i} \Bigg \vert - \sum_{a = 0}^{r_i - 1} O \bigg ( \frac {a^2} {h_i^2} \bigg ) \prod_{j = 0, j \ne a}^{r_i - 1} \exp \bigg ( \frac {-j}{h_i} \bigg ) +
    \sum_{a = 0}^{r_i - 1} \sum_{b = a + 1}^{r_i - 1} O \bigg ( \frac {a^2b^2} {h_i^4} \bigg ) \prod_{j = 0, j \ne a, j \ne b}^{r_i - 1} \exp \bigg ( \frac {-j}{h_i} \bigg ) - \cdots \Bigg \vert
\end{align*}
...
\begin{align*}
           & \frac {r_i} {h_i} \Bigg \vert \sum_{a = 0}^{r_i - 1} O \bigg ( \frac {a^2} {h_i^2} \bigg ) +
    \sum_{a = 0}^{r_i - 1} \sum_{b = a + 1}^{r_i - 1} O \bigg ( \frac {a^2b^2} {h_i^4} \bigg ) + \sum_{a = 0}^{r_i - 1} \sum_{b = a + 1}^{r_i-1} \sum_{c = b + 1}^{r_i - 1} O \bigg ( \frac {a^2b^2c^2} {h_i^6} \bigg ) + \cdots \Bigg \vert \\
    \le \, & \frac {r_i} {h_i} \Bigg \vert \, r_i \ O \bigg ( \frac {r_i^2} {h_i^2} \bigg ) +
    r_i^2 \, O \bigg ( \frac {r_i^4} {h_i^4} \bigg ) + r_i^3 \,  O \bigg ( \frac {r_i^6} {h_i^6} \bigg ) \cdots \Bigg \vert                                                                                                                  \\
    < \,   & \frac {r_i} {h_i} \Bigg \vert \, \ O \bigg ( \frac 1 {h_i^{1/5}} \bigg ) + \, O \bigg ( \frac 1 {h_i^{2/5}} \bigg ) +  O \bigg ( \frac 1 {h_i^{3/5}} \bigg ) + \cdots \Bigg \vert
\end{align*}
was klar gegen 0 geht, wenn $h_i \to \infty$. (Eigentlich lassen wir $p \to \infty$ gehen, aber da $h_i = p/(\gcd(p - 1, 2k_i) - 1)$ gilt $h_i = \Theta(p)$ und damit $h_i \to \infty \Longleftrightarrow p \to \infty$.) Im Fall $r_i < h_i^{3/5}$ können wir also
\begin{align*}
    \P(s_i = r_i)
     & \sim \frac {r_i} {h_i} \prod_{j = 0}^{r_i - 1} \exp \bigg ( \frac {-j}{h_i} \bigg )
    \frac {r_i} {h_i} \exp \sum_{j = 0}^{r_i - 1} \frac {-j} {h_i}                         \\
     & = \frac {r_i} {h_i} \exp \frac {-r_i (r_i - 1)} {2h_i}                              \\
     & \sim \frac {r_i} {h_i} \exp \frac {-r_i^2} {2h_i}
\end{align*}
schreiben. Im Fall $r_i > h_i^{3/5}$ gilt
\begin{align*}
    \P(s_i = r_i)
     & = \frac {r_i} {h_i} \prod_{j = 0}^{r_i - 1} \exp \bigg ( \frac {-j}{h_i} \bigg )                                   \\
     & \le \frac {r_i} {h_i} \prod_{j = 0}^{\big \lfloor h_i^{3/5} \big \rfloor - 1} \exp \bigg ( \frac {-j}{h_i} \bigg ) \\
     & \sim \frac {r_i} {h_i} \exp \frac {- \Big \lfloor h_i^{3/5} \Big \rfloor^2 } {2h_i}                                \\
     & \le \frac {r_i} {h_i} \exp \frac {- h_i^{1/5}} {2}
\end{align*}
Da immer $r_i < h_i$ gilt, geht das gegen 0 für $h_i \to \infty$. Da $\min_{i = 1}^M (r_i \lg 2k_i)$ durch $p$ beschränkt ist, folgt direkt, dass Terme in (\ref{etmin}) gegen 0 gehen, in denen $r_i > h_i^{2/3}$ gilt. Insgesamt folgt daraus
\begin{align*}
    \E(t_{\min}) \sim \sum_{r_1 = 1}^{h_1} \frac {r_1} {h_1} \exp \frac {-r_1^2} {2h_1} \, \cdots \sum_{r_M = 1}^{h_M} \frac {r_M} {h_M} \exp \frac {-r_M^2} {2h_M} \, \bigg ( \min_{i = 1}^M {r_i \lg 2k_i} \bigg )
\end{align*}
...
\begin{align*}
    \E(t_{\min})
     & \sim \int_{1}^{h_1} \frac {r_1} {h_1} \exp \bigg (\frac {-r_1^2} {2h_1} \bigg ) \cdots \int_{1}^{h_M} \frac {r_M} {h_M} \exp \bigg ( \frac {-r_M^2} {2h_M} \bigg ) \bigg ( \min_{i = 1}^M {r_i \lg 2k_i} \bigg ) \ d r_M \cdots dr_1        \\
     & \sim \int_{1}^{\infty} \frac {r_1} {h_1} \exp \bigg ( \frac {-r_1^2} {2h_1} \bigg ) \cdots \int_{1}^{\infty} \frac {r_M} {h_M} \exp \bigg ( \frac {-r_M^2} {2h_M} \bigg ) \bigg ( \min_{i = 1}^M {r_i \lg 2k_i} \bigg ) \ d r_M \cdots dr_1
\end{align*}
da bereits gezeigt wurde, dass Terme in denen $r_i > h_i^{3/5}$ für den asymptotischen Wert von $\E(t_{\min})$ irrelevant sind. Für den nächsten Schritt ist es nötig, dass in jedem Integral über $t_i$ integriert wird. Da aber $r_i \lg 2k_i = t_i$, kann das durch eine einfache Variablensubstitution erreicht werden. Da $ dr_i = dt_i / \lg 2k_i$ gilt
\begin{align*}
    \E(t_{\min}) \sim \int_{1}^{\infty} \frac {t_1} {h_1 \lg 2k_1} \exp \frac {-t_1^2} {2h_1 \lg^2 2k_1} \, \cdots \int_{1}^{\infty} \frac {t_M} {h_M \lg 2k_M} \exp \frac {-t_M^2} {2h_M \lg^2 2k_M} \, \bigg ( \min_{i = 1}^M {t_i} \bigg ) \ \frac {dt_M \cdots dt_1} {\lg 2k_M \cdots \lg 2k_1}
\end{align*}
