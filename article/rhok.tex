...
\begin{align*}
    ...
    \, \min_{i = 1}^M(z_i \lg 2k_i)
\end{align*}
Mit der Notation von Lemma \ref{lemma:prob-s-z} gilt wegen $1 + x \le \exp(x)$
\begin{align*}
    \P(s = z)
    = \frac z h  \prod_{j = 0}^{z - 1} \bigg (1 - \frac j h \bigg )
    \le \frac z h \prod_{j = 0}^{z - 1} \exp \bigg ( \frac {-j} h \bigg )
    = \frac z h \exp \Bigg ( \sum_{j = 0}^{z - 1} \frac {-j} h \Bigg )
    = \frac z h \exp \bigg (\frac {-z(z - 1)} {2h} \bigg )
\end{align*}
Daraus folgt $\P(s = z) = O(Q(z, h))$. Das Ziel ist nun zu zeigen, dass $\E(t_{\min}) \sim F$. Eine Tatsache, die dafür ohne Beweis verwendet wird, ist, dass ein $\delta > 0$ existiert, sodass $\E(t_{\min}) > p^{2\delta}$. Das sollte aber intuitiv sein, da die erwartete Laufzeit sonst subexponentiell wäre.

\begin{lemma}
    Sei $0 < \delta < 1/2$. Mit der Notation von Lemma \ref{lemma:prob-s-z} gilt für $p \to \infty$ und $z \le h^{1/2 + \delta}$
    \begin{align*}
