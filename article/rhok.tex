...
\begin{lemma}
    Man betrachte eine Maschine mit Parameter $k$ und $h = p/(\gcd(p - 1, 2k) - 1)$. Sei $\P(s = z)$ die Wahrscheinlichkeit, dass bei dieser Maschine nach $z$ Iterationen erstmals eine Kollision auftritt. Für $p \to \infty$ gilt
    \begin{align*}
        \P(s = z)
        \sim \frac z h \exp \Bigg ( \frac {-z^2}{2h} \Bigg )
    \end{align*}
    wenn $z = O(\sqrt h)$ und
    \begin{align*}
        \P(s = z)
        = O \Bigg ( \frac z h \exp \Bigg ( \frac {-z^2}{2h} \Bigg ) \Bigg )
    \end{align*}
    wenn $z = \omega(\sqrt h)$.
...
\begin{remark*}
    $z$ wird hier als Funktion von $h$ betrachtet. Denn um (\ref{formula:etmin-initial}) später für $p \to \infty$ auszuwerten, reicht eine Abschätzung von $\P(s_i = z_i)$ für konstante $z_i$ nicht aus, da über alle $1 \le z_i \le h_i$ summiert wird und $h_i = p/(\gcd(p - 1, 2k_i) - 1)$.
\end{remark*}

\begin{proof}[Beweis von Lemma \ref{lemma:prob-s-z}]
    Nach RMA genügt es $\P(\nu(f, x_0) = z)$ für eine zufällig gewählte Funktion $f \in A(h_i)$ und ein zufälliges $x_0 \in \Z/h_i\Z$ zu bestimmen. Für eine zufällige Funktion $f \in A(h_i)$ ist die Wahrscheinlichkeit einer Kollision im $j$-ten Schritt $j/h_i$, wenn in den ersten $j - 1$ Schritten keine Kollision aufgetreten ist, da jeder der $h_i$ möglichen Werte gleich wahrscheinlich ist und $j$ von ihnen zu einer Kollision führen. Es gilt also
    \begin{align*}
        \P(s_i = z) = \P(\nu(f, x_0) = z)
        = \frac{z}{h_i} \, \prod_{j = 0}^{z - 1} \bigg (1 - \frac {j} {h_i} \bigg )
    \end{align*}
    da in den ersten $z - 1$ Schritten keine Kollision auftreten darf und im $z$-ten Schritt eine Kollision auftreten muss. MAKE IT SIMPLE
\end{proof}
