...
\begin{definition}
    Sei $f : X \to X$ eine Abbildung über einer endlichen Menge $X$ und $x \in X$. Wir nennen
    \begin{enumerate}
        \item $\mu(f, x)$ die Höhe von $x$ in seinem Baum in $\gamma(f)$.
        \item $\lambda(f, x)$ die Länge des Zyklus in der Zusammenhangskomponente von $x$ in $\gamma(f)$.
        \item $\rho(f, x) = \mu(f, x) + \lambda(f, x)$ die Rho-Länge von $x$.
    \end{enumerate}
\end{definition}
Als Beispiel betrachte man $f$ aus Abbildung \ref{fig:fn-graph-17}. Für $x = 13$ gilt $\mu(f, 13) = 3, \lambda(f, 13) = 6$ und $\rho(f, 13) = 9$. Für $x = 15$ gilt $\mu(f, 15) = 1, \lambda(f, 15) = 6$ und $\rho(f, 15) = 7$. Hier tritt also nach siebenmaliger Anwendung von $f$ mit Anfangswert 15 erstmals eine Kollision auf.

\begin{figure}
    \centering
    \begin{tikzpicture}
        \newcommand{\cNode}{magenta}
        \newcommand{\cEdge}{magenta}
        ...
        \node[circle, draw=\cNode] (2) at (3, 0) {2};
        \node[circle, draw=\cNode] (5) at (4, -1.3) {5};
        \node[circle, draw=\cNode] (9) at (5.5, -1.3) {9};
        \node[circle, draw=\cNode] (14) at (6.5, 0) {14};
        \node[circle, draw=\cNode] (10) at (5.5, 1.3) {10};
        \node[circle, draw=\cNode] (16) at (4, 1.3) {16};
        \node[circle, draw=black] (4) at (-1.1, -1.1) {4};
        \node[circle, draw=black] (13) at (-1.1, 1.1) {13};
        \node[circle, draw=\cNode] (15) at (4, -2.7) {15};
        ...
        \draw[-latex] (0) -- (1);
        \draw[-latex] (1) -- (2);
        \draw[-latex, draw = \cEdge] (2) -- (5);
        \draw[-latex, draw = \cEdge] (5) -- (9);
        \draw[-latex, draw = \cEdge] (9) -- (14);
        \draw[-latex, draw = \cEdge] (14) -- (10);
        \draw[-latex, draw = \cEdge] (10) -- (16);
        \draw[-latex, draw = \cEdge] (16) -- (2);
        \draw[-latex] (13) -- (0);
        \draw[-latex] (4) -- (0);
        \draw[-latex] (7) -- (16);
        \draw[-latex] (3) -- (10);
        \draw[-latex, draw = \cEdge] (15) -- (5);
        \draw[-latex] (12) -- (9);
        \draw[-latex] (6) -- (3);
        \draw[-latex] (11) -- (3);
        \draw[-latex] (8) -- (14);
    ...
    \caption{Der funktionale Graph von $f : \Z/17\Z \to \Z/17\Z$ mit $f : x \mapsto x^2 + 1$. Der Weg, den die Folge $x_0, f(x_0), f(f(x_0)), f(f(f(x_0))), \dots$ mit $x_0 = 15$ beschreibt, ist farbig markiert. Konkret lautet die Folge $15, 5, 9, 14, 10, 16, 2, 5, 9, 14, 10, 16, 2, \dots$}
    \label{fig:fn-graph-17}
\end{figure}

\subsection{Kollisionserkennung im funktionalen Graphen modulo $\boldsymbol{p}$.}

Sei $f$ die Abbildung $g$, betrachtet in $\Z/p\Z$ und $x_i' = \pi(x_i)$ für alle $i \in \N$. Die Folge $(x_{i}')_{i \in \N}$ "`läuft"' analog durch $\gamma(f)$, beginnend von $x_0'$. Unser Ziel, das Finden einer Kollision von $(x_i')_{i \in \N}$, ist also äquivalent dazu, einen Knoten in $\gamma(f)$ zu finden, der zweimal in dem von $(x_i')_{i \in \N}$ abgelaufenen Pfad erscheint. Dafür kann Floyds Algorithmus zur Zykluserkennung in $\gamma(f)$ verwendet werden. Floyds Algorithmus macht sich zunutze, dass es ein $1 \le r \in \N$ mit $x_r' = x_{2r}'$ geben muss (\cite{knu98}, S. 7). Für das minimale solcher $r$ gilt $r \le \rho(f, x_0')$ (\cite{knu98}, S. 7). Als Beispiel betrachte man wieder $f$ aus Abbildung \ref{fig:fn-graph-17} und $x_0 = 15$. Das minimale $r$ mit $x_r = x_{2r}$ ist $r = 6$, und es gilt $x_r = 2$.

Es also genügt also, die Folgen $(x_i')_{i \in \N}$ und $(x_{2i}')_{n \in \N}$ gleichzeitig Glied für Glied zu berechnen und in jedem Schritt zu überprüfen, ob $x_i' = x_{2i}'$. Das kann natürlich nicht explizit geschehen, da $p$ unbekannt ist. Stattdessen werden $(x_i)_{i \in \N}$ und $(x_{2i})_{i \in \N}$ Glied für Glied berechnet. Das Überprüfen, ob $x_i' = x_{2i}'$ geschieht durch Berechnung von $\gcd(n, x_i - x_{2i})$. So erhält man nach maximal $\rho(f, x_0')$ Schritten die gewünschte Kollision. Die Rho-Methode ist in Algorithmus \ref{alg:rho} dargestellt. Nach der $i$-ten Iteration speichert $x$ das $i$-te Glied der Folge $(x_i)_{i \in \N}$ und $y$ das $i$-te Glied von $(x_{2i})_{i \in \N}$.
\begin{algorithm}
...
\end{align}
Indem man die Näherung $1 + x \approx e^x$ für kleine $x$ verwendet, sieht man, dass $\P(Z_i = z) \approx (z/h_i) \prod_{j = 1}^{z - 1} e^{-j/h_i} \approx Q(z, h_i)$. Damit gilt auch $F \approx L_{(k_i)}(p)$. \glqq $\approx$\grqq \space hat im Gegensatz zu \glqq$\sim$\grqq \space keine formale Bedeutung, sondern dient lediglich der Motivation der folgenden Schritte. Das Ziel ist nun, eine einfache asymptotische Näherung für $F$ zu finden und zu zeigen, dass $L_{(k_i)}(p) \sim F$.
...
\begin{theorem}
    ...
    Sei $L_k(p)$ wie in (\ref{eq:lki-definition}) definiert. Der Erwartungswert $\E(L_k(p))$ über alle möglichen $\gcd((p - 1)/2, k)$ ist für $k = 1$ minimal.
\end{theorem}
...
\begin{figure}
    \centering
    \begin{tikzpicture}
        \draw[draw = red] (4.45, 2.38) circle [x radius=0.5, y radius=0.2, rotate = 325];
        \draw[draw = red] (4.45, 3.57) circle [x radius=0.5, y radius=0.2, rotate = 25];
...
\begin{figure}
    ...
    \begin{tabular}{ccc|c|c}
        $k_1$ & $k_2$ & $k_3$ & Laufzeit                         & Formel (\ref{eq:lki-asymp})      \\
        \hline
        1     & 1     & 1     & \textcolor[HTML]{ 0020ff }{1.00} & \textcolor[HTML]{ 0020ff }{1.00} \\
        1     & 1     & 2     & \textcolor[HTML]{ 1620e8 }{1.07} & \textcolor[HTML]{ 1420ea }{1.10} \\
        1     & 1     & 3     & \textcolor[HTML]{ 1f20df }{1.10} & \textcolor[HTML]{ 1d20e1 }{1.14} \\
        1     & 1     & 4     & \textcolor[HTML]{ 1f20df }{1.10} & \textcolor[HTML]{ 1d20e1 }{1.14} \\
        1     & 2     & 2     & \textcolor[HTML]{ 3220cc }{1.16} & \textcolor[HTML]{ 3420ca }{1.25} \\
        1     & 2     & 3     & \textcolor[HTML]{ 3e20c0 }{1.20} & \textcolor[HTML]{ 3c20c2 }{1.29} \\
        1     & 2     & 4     & \textcolor[HTML]{ 4520b9 }{1.22} & \textcolor[HTML]{ 4020be }{1.31} \\
        1     & 3     & 3     & \textcolor[HTML]{ 5b20a3 }{1.29} & \textcolor[HTML]{ 4f20af }{1.38} \\
        1     & 3     & 4     & \textcolor[HTML]{ 5b20a3 }{1.29} & \textcolor[HTML]{ 4b20b3 }{1.36} \\
        1     & 4     & 4     & \textcolor[HTML]{ 61209d }{1.31} & \textcolor[HTML]{ 5120ad }{1.39} \\
        2     & 2     & 2     & \textcolor[HTML]{ 6b2093 }{1.34} & \textcolor[HTML]{ 792085 }{1.58} \\
        2     & 2     & 3     & \textcolor[HTML]{ 7a2084 }{1.39} & \textcolor[HTML]{ 81207d }{1.62} \\
        2     & 2     & 4     & \textcolor[HTML]{ 81207d }{1.41} & \textcolor[HTML]{ 94206a }{1.71} \\
        2     & 3     & 3     & \textcolor[HTML]{ 972067 }{1.48} & \textcolor[HTML]{ 9c2062 }{1.75} \\
        2     & 3     & 4     & \textcolor[HTML]{ 9d2061 }{1.50} & \textcolor[HTML]{ a0205e }{1.77} \\
        2     & 4     & 4     & \textcolor[HTML]{ a62058 }{1.53} & \textcolor[HTML]{ be2040 }{1.91} \\
        3     & 3     & 3     & \textcolor[HTML]{ e82016 }{1.74} & \textcolor[HTML]{ e82016 }{2.11} \\
        3     & 3     & 4     & \textcolor[HTML]{ e2201c }{1.72} & \textcolor[HTML]{ ca2034 }{1.97} \\
        3     & 4     & 4     & \textcolor[HTML]{ e2201c }{1.72} & \textcolor[HTML]{ d1202d }{2.00} \\
        4     & 4     & 4     & \textcolor[HTML]{ ff2000 }{1.81} & \textcolor[HTML]{ ff2000 }{2.22} \\
    \end{tabular}

    \caption{Laufzeitmessungen und theoretische Werte von Formel (\ref{eq:lki-asymp}) für $M = 3$ und $1 \le k_1 \le k_2 \le k_3 \le 4$.}
    \label{fig:measurements-m3}
\end{figure}

Um zu demonstrieren, dass Satz \ref{theorem:lki-asymp} das Laufzeitverhalten von Pollards Rho-Algorithmus gut beschreibt, wurden Laufzeitmessungen für $M = 1, M = 2$ und $M = 3$ durchgeführt. Als Testzahlen wurden 192-Bit-Zahlen verwendet, bei denen jeder Primfaktor größer als $2^{22}$ ist und ein Primfaktor kleiner als $2^{23}$ ist. Die Größe der Primfaktoren nach unten zu beschränken ist sinnvoll, da die Laufzeit der Rho-Methode sonst sehr kurz wäre und die Messergebnisse durch Dinge wie den Aufwand der Laufzeitmessung selbst verfälscht werden würden. Es ist auch sinnvoll, dass die Testzahlen einen kleinen Primfaktor enthalten, da die Laufzeit sonst sehr groß wäre. Es wurden jeweils $2^{14} = 16384$ Testzahlen verwendet. Die mittlere Laufzeit und Werte von (\ref{eq:lki-asymp}) zum Vergleich sind in den Abbildungen \ref{fig:measurements-m1}, \ref{fig:measurements-m2} und \ref{fig:measurements-m3} dargestellt. Sowohl die Werte der Formel als auch die Messdaten wurden skaliert, sodass bei $k_i = 1$ für alle $1 \le i \le M$ der Wert 1 steht. Der Code für die Laufzeitmessungen ist unter \href{https://github.com/finn-rudolph/rhok}{\texttt{https://github.com/finn-rudolph/rhok}} verfügbar.
