...
\end{align*}
Da die $i$-te Maschine in $t$ Zeiteinheiten $\lfloor t / \lg 2k_i \rfloor$ Schritte ausführt, gilt $\P(t_i > t) \sim \P(s_i > t / \lg 2k_i)$ für große $t$. Das Weglassen der Gaußklammern lässt sich damit rechtfertigen, dass für ausreichend großes $p$ nur große $t$ relevant für den Erwartungswert von $t_{\min}$ sind, bei denen es kaum einen Unterschied macht. Zur Herleitung der Formel werden noch weitere solcher Vereinfachungen nötig sein. Sie lassen sich alle rigoros begründen, was jedoch meist aufwändig und uninteressant ist. Im Abschnitt \emph{Weglassen von $O(j^2/h_i^2)$} wird für eine Näherung ein Beweis vorgestellt, um zu zeigen, wie ein solches Argument aussehen kann. Nun wird die Restgliedabschätzung $\exp x = 1 + x + O(x^2)$ für $|x| < 1$ mit $x = -j/h_i$ angewandt.
\begin{align*}
    \P(s_i > s)
...
\paragraph{$\boldsymbol{\pmb{\P}_{t_i > t - 1 \, \forall i}(t_i = t}$  für mindestens ein $\boldsymbol{i}$).} Die Wahrscheinlichkeit, dass bei Maschine $i$ eine Kollision nach genau $t$ Zeiteinheiten auftritt ist
    ...
    \paragraph{$\pmb{M = 2}$.} Auch hier wurde jedes zusammengesetzte $2^{60} \le n \le 2^{60} + 2^{16}$ jeweils 50 Mal Pollards Rho-Algorithmus ausgeführt. Es wurden jeweils zwei zufällige Anfangswerte gewählt das Minimum der Laufzeit für beide Anfangswerte betrachtet. Da Zahlen in der Größenordnung $2^{60}$ vom Rho-Algorithmus meist in weniger als 1 ms faktorisiert werden, wurde er nicht echt parallel ausgeführt, sondern zweimal hintereinander und zweimal die Laufzeit gemessen. Denn bei so geringer Laufzeit würde die Zeit, um beispielsweise zwei Threads zu erstellen, das Messergebnis verfälschen. Die Ergebnisse für $1 \le k_1 \le k_2 \le 12$ sowie entsprechende Werte von (\ref{expectation-tmin}) zum Vergleich sind in ? dargestellt.

\begin{figure}
    \begin{tabular}{cc}
        \small
        \begin{tabular}{cccccccc}
            \textcolor[HTML]{ 0020ff }{ 0.71 } & \textcolor[HTML]{ 1320eb }{ 0.83 } & \textcolor[HTML]{ 1b20e3 }{ 0.87 } & \textcolor[HTML]{ 1c20e2 }{ 0.88 } & \textcolor[HTML]{ 2220dc }{ 0.91 } & \textcolor[HTML]{ 1d20e1 }{ 0.88 } & \textcolor[HTML]{ 2520d9 }{ 0.93 } & \textcolor[HTML]{ 2120dd }{ 0.91 } \\
                                               & \textcolor[HTML]{ 3a20c4 }{ 1.06 } & \textcolor[HTML]{ 4e20b0 }{ 1.18 } & \textcolor[HTML]{ 5020ae }{ 1.19 } & \textcolor[HTML]{ 62209c }{ 1.30 } & \textcolor[HTML]{ 5320ab }{ 1.21 } & \textcolor[HTML]{ 6b2093 }{ 1.35 } & \textcolor[HTML]{ 5f209f }{ 1.28 } \\
                                               &                                    & \textcolor[HTML]{ 6e2090 }{ 1.37 } & \textcolor[HTML]{ 71208d }{ 1.39 } & \textcolor[HTML]{ 8f206f }{ 1.57 } & \textcolor[HTML]{ 752089 }{ 1.42 } & \textcolor[HTML]{ 9f205f }{ 1.67 } & \textcolor[HTML]{ 8a2074 }{ 1.54 } \\
                                               &                                    &                                    & \textcolor[HTML]{ 752089 }{ 1.42 } & \textcolor[HTML]{ 94206a }{ 1.60 } & \textcolor[HTML]{ 7a2084 }{ 1.44 } & \textcolor[HTML]{ a62058 }{ 1.71 } & \textcolor[HTML]{ 90206e }{ 1.57 } \\
                                               &                                    &                                    &                                    & \textcolor[HTML]{ c1203d }{ 1.87 } & \textcolor[HTML]{ 9a2064 }{ 1.64 } & \textcolor[HTML]{ dc2022 }{ 2.04 } & \textcolor[HTML]{ bb2043 }{ 1.84 } \\
                                               &                                    &                                    &                                    &                                    & \textcolor[HTML]{ 7e2080 }{ 1.47 } & \textcolor[HTML]{ ae2050 }{ 1.76 } & \textcolor[HTML]{ 962068 }{ 1.61 } \\
                                               &                                    &                                    &                                    &                                    &                                    & \textcolor[HTML]{ ff2000 }{ 2.24 } & \textcolor[HTML]{ d62028 }{ 2.00 } \\
                                               &                                    &                                    &                                    &                                    &                                    &                                    & \textcolor[HTML]{ b62048 }{ 1.80 } \\
        \end{tabular}
