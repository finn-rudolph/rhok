...
\begin{proof}
    ...
    \begin{align*}
        Y = \prod_{i = 1}^r Y_i
    \end{align*}
    Es gilt $\mathbb{P}(Y_i = d) = 0$, wenn $d$ keine Potenz von $q_i$ ist. Andernfalls gilt nach Lemma 1
    \begin{equation*}
        \mathbb{P}(Y_i = q_i^{h})
        = \frac {\varphi(q_i^{e_i} / q_i^h)}{q_i^{e_i}}
        = \frac{\varphi(q_i^{e_i - h})}{q_i^{e_i}}
        = \begin{dcases*}
            \frac {q_i - 1} {q_i^{h + 1}} & \quad $h < e_i$ \\
    ...
    \begin{align*}
        \E(Y)
         & = \E \left ( \prod_{i = 1}^r Y_i \right ) = \prod_{i = 1}^r \E(Y_i) \\
         & = \prod_{i = 1}^r \sum_{j = 0}^{e_i} \mathbb{P}(\gcd(p - 1)/2, k)
    \end{align*}
\end{proof}
