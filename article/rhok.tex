...
\label{lemma-prob-si}
\begin{lemma}
    Sei $\P(s_i = z)$ die Wahrscheinlichkeit, dass bei der $i$-ten Maschine nach $z$ Iterationen erstmals eine Kollision auftritt. Für $p \to \infty$ gilt
    \begin{align*}
        \P(s_i = z) \sim \frac z {h_i} \exp \bigg ( \frac {-z^2}{2h_i} \bigg )
    \end{align*}
\end{lemma}
...
\begin{proof}
    ...
    \end{align*}
    da in den ersten $z - 1$ Schritten keine Kollision auftreten darf und im $z$-ten Schritt eine Kollision auftreten muss. Durch Anwenden der Restgliedabschätzung $\exp (x) = 1 + x + O(x^2)$ (gegeben $|x| < 1$) auf $x = -j/h_i$ gilt
    \begin{align*}
        \P(s_i = z)
        = \frac {z} {h_i} \, \prod_{j = 0}^{z - 1} \Bigg ( \exp \bigg ( \frac {-j}{h_i} \bigg )
        - O \bigg ( \frac {j^2} {h_i^2} \bigg ) \Bigg )
    \end{align*}
    Um die gewünschte Formel zu erhalten, wird gezeigt, dass der $O(j^2/h_i^2)$-Term für eine asymptotische Näherung weggelassen werden kann.
\end{proof}
