...
where
    T: Default + Clone + 'a + AddAssign<&'a T> + Sub<Output = T>,
{
    pub fn new(n: usize) -> FenwickTree<T> {
        FenwickTree(vec![Default::default(); n])
    }

    pub fn update(&mut self, i: usize, delta: &'a T) {
    ...
    pub fn prefix_sum(&'a self, i: usize) -> T {
    ...
    pub fn range_sum(&'a self, i: usize, j: usize) -> T {
        if i == 0 {
            self.prefix_sum(j)
        } else {
